    ...
    try {
      ...
      if (response && response.data) {
        ...
        if (!student.name || student.name === 'N/A' || !student.indexNumber || 
          ...
          // Try to fetch more complete student data
          try {
            const studentId = student._id || qrData._id || qrData.id || qrData;
            if (studentId) {
              const moreDataResponse = await attendanceService.getStudentById(studentId);
              if (moreDataResponse.success && moreDataResponse.data) {
                // Merge the additional data with the student object
                Object.assign(student, moreDataResponse.data);
                
                // Update the response data with the enhanced student info
                if (response.data) {
                  response.data.student = student;
                }
                
                console.log('Retrieved additional student data:', student);
              }
            }
          } catch (dataError) {
            console.error('Failed to get additional student data:', dataError);
          }
        }
        ...
        if (onScanSuccess) {
          onScanSuccess({
            ...
            attendance: {
              ...
              whatsAppNotification: attendance.whatsAppNotification || 'Not Sent'
            },
            email: student.email || student.student_email || 'N/A',
            address: student.address || 'N/A',
            messageStatus: attendance.whatsAppNotification === 'Sent' ? 'sent' : 'not sent'
          });
        }
      } else {
