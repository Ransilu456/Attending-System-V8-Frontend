...
import { Smartphone, QrCode, RefreshCw, Camera, Upload } from 'lucide-react';
...
const QRScannerPage = () => {
  ...
  const handleScanSuccess = useCallback(async (result) => {
    try {
      ...
      // The scanner components now pass student data directly
      ...
      const displayData = {
        ...
        email: student.student_email || student.email || 'N/A', // Fall back to student_email for compatibility 
        ...
        messageStatus: attendance.whatsAppNotification === 'Sent' ? 'sent' : 'pending'
      };
      ...
      ToastHelper.success(`Attendance marked for ${displayData.name || 'Student'} as ${status}`);
